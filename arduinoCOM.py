...
def read_from_port(serial_port, name):
    global auth, keypad_auth, rf_auth
    while True:
        if serial_port.in_waiting > 0:
            ...
            with auth_lock:
                if data == 'BoxOpen':  # If 'BoxOpen' is received, set auth to 2
                    auth = 2
                    print(f"Received 'BoxOpen' from {name}, auth set to 2.")

                elif data == 'V':  # If 'V' is received, reset auth and send '1'
                    auth = 0
                    keypad_auth = 0
                    rf_auth = 0
                ...
                elif data == 'B' and rf_auth == 0:  # If 'B' is received and rf_auth is 0
                    auth += 1
                    rf_auth = 1
                    print(f"Received 'B' from {name}, auth incremented to {auth}, rf_auth set to 1.")
...
def main():
    ...
    potential_ports = ['COM1', 'COM2', 'COM3', 'COM4', 'COM5']  # List of potential COM ports
